...
void DeleteFF(ffentryPtr p) {
    ...
    if (prev == NULL) {
        fprintf(stderr, "entry not found in DeleteFF\n");
        return;
    }
    if (p->pathnameGS != NULL)
        nfree(p->pathnameGS);
    if (p->pathnameP != NULL)
        nfree(p->pathnameP);
...
ffentryPtr FindFF(GSString255Ptr path) {
    ...
    p = ffList;
    while (p != NULL) {
        if (GScaseEqual(path, p->pathnameGS) && (!p->hidden))
            break;
        p = p->next;
    }
    return p;
...
void ffErr(int err) { /*   fprintf(stderr,"FastFile error: %04X\n",err); */ }
...
int fastEntry(FastFilePB *ff, int osFlag, int pCount) {
    ...
    case 2: /* Load from Memory */
        ...
        if (p == NULL) {
            if (ff->action == 2) {
                ...
                p = NewFF(pathCopy); /* make a new entry */
        }
    ...
    retryil:
        ...
        if ((*p->fileHandle != NULL)) {         /*&& /* handle purged?    */
            ...
            if (!osFlag)
                ...
                copygs2res(fg->pathName, p->pathnameGS);
            }
            HLock(p->fileHandle);
            ff->flags = p->flags;
            break;
    ...
    case 3: /* Save */
        ...
        cr.auxType = p->info.auxType = (osFlag ? fg->auxType : ff->aux_type);
    ...
    if (!err)
        ...
        case 2:
            if (!osFlag) {
                ...
                switch (pCount) {
                case 14:
                    fg->blocksUsed = p->info.blocksUsed;
                case 13:
                    fg->fileLength = p->info.eof;
                case 12:
                    fg->option = p->info.optionList;
                case 11:
                    memcpy(&fg->modDate, &p->info.modDateTime, sizeof(TimeRec));
                case 10:
                    memcpy(&fg->createDate, &p->info.createDateTime,
                           sizeof(TimeRec));
                case 9:
                    fg->storageType = p->info.storageType;
                case 8:
                    fg->auxType = p->info.auxType;
                case 7:
                    fg->fileType = p->info.fileType;
                case 6:
                    fg->access = p->info.access;
                default:
                    fg->fileHandle = p->fileHandle;
                    if (ff->action == 1)
                        fg->flags = p->flags;
                }
        }
