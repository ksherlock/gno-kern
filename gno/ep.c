    ...
    if (np != NULL) {
        while (!feof(np)) {
            ...
            while ((*pt != 0) && (!isspace(*pt)))
                pt++;
            if (*pt == 0)
                PANIC("invalid namespace specifier");
        ...
        printf("warning: could not locate :etc:namespace\n");
    free(line);
}
...
Gstr match(Gstr fname, unsigned short leng, char sep) {
    ...
    while (l != NULL) {
        ...
        if ((l->pfx->length == i) &&
            (!strincmp(fname->text + 1, l->pfx->text + 1, i - 1)))
            break;
        l = l->next;
    }
    if (l == NULL)
...
stackPtr pop(void) {
    if (!stack_ind)
        ...
        return &the_stack[--stack_ind];
}

void clearstack(void) { stack_ind = 0; }
...
Gstr gno_ExpandPath(Gstr i_path, int num, word npFlag) {
    ...
    if (i_path->length > 1024)
        return (Gstr)0xFFFF0040l;
    g_out = go;
    rbuf.bufSize = 32;
    ep.pCount = 3;
    ep.flags = 0;
    ...
    i = 0;
    while ((i < i_path->length) && !isSeparator(i_path->text[i]))
        i++;
    if (i == i_path->length)
        SEP = ':';
    else
        SEP = i_path->text[i]; /* grab the separator character */
    ...
    if (!isSeparator(i_path->text[0])) {
        ...
        i = 0;
        while ((i < i_path->length) && (i_path->text[i] != SEP))
            i++;
...
phase2:
    ...
    if (!npFlag) {
        ...
        /*  SEP = top()->sep; */
        i = 1;
        while ((i < g->length) && (g->text[i] != top()->sep))
            i++;
    ...
    while (sp = pop()) {
        ...
        while (i < oldlen) {
            ...
            while ((i < oldlen) && (g->text[i] != nSEP))
                i++;
            if ((i - j == 1) && (g->text[j] == '.'))
                outind--;
            else if ((i - j == 2) && (g->text[j] == '.') &&
                     (g->text[j + 1] == '.')) {
                outind2 = outind - 2;
                while ((outind2 > 0) && (g_out->text[outind2] != ':'))
                    outind2--;
                if (outind2 != 0)
                    outind = outind2; /* move outind back */
                else
                    outind--; /* just zap the colon */
            } else
                while (j < i)
                    g_out->text[outind++] = g->text[j++];
        }
        i = sp->ind;
    }
    g_out->length = outind;
