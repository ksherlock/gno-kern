...
static int createProc(word stack, word dp, longword process) {
    ...
    kp->procTable[p].irq_B = kp->procTable[p].irq_K = (process >> 16);
...
pascal int KERNSetGNOQuitRec(word pCount, GSString255Ptr pathname, word flags,
                             int *ERRNO) {
...
int commonFork(void (*funcptr)(), word stackSize, int prio, char *name,
    ...
    *ret = ((longword)endproc2) - 1; /* go to endproc when process is done */
    ...
    disableps();
    newPID = createProc(dPageAddr + buffSize - 5 - nargs, dPageAddr,
                        (longword)funcptr);
    if (newPID == SYSERR) {
        *ERRNO = EAGAIN;
        DisposeHandle(fstack);
        enableps();
        return -1;
    }
 ...
 */
static void kill_self(void) { Kkill(&errno, 9, PROC->flpid); }
...
#define r_rec il_out.r
    ...
    if (toolerror() == 0x1101) {
        ...
        /* if the 'newID' is being used by a process, force another copy from
         * disk */
        for (i = 0; i < NPROC; i++) {
            if (kp->procTable[i].processState != procUNUSED)
    ...
    *ret = ((longword)endproc2) - 1; /* go to endproc when process is done */
    ...
    if (p->args)
        nfree(p->args); /* free the args string of the old executable */
    p->args = args;
    if (fi.fileType != 0xB5)
        argscopy = NULL;
    ...
    for (ind = resBuf->bufString.length - 1; ind > 0; ind--)
        if (resBuf->bufString.text[ind] == ':') {
            resBuf->bufString.length = ind + 1;
            break; /* +1 -> snag the colon */
    ...
    if ((fi.auxType == 0xDC00) && (p->ttyID != 3)) {
        ...
        if (oldFlags & FL_FORKED) {
            ...
            UserShutDown(oldUserID,
                         ((oldFlags & FL_RESTART) && !(oldFlags & FL_NORESTART))
                             ? 0x4000
                             : 0);
        }
...
int KERNtctpgrp(int *ERRNO, int pid, int fdtty) {
    ...
    /*
       if (!((mpid == Kgetpid()) || (kp->procTable[mpid].parentpid ==
       Kgetpid()))) { *ERRNO = ERANGE; return -1; }
    */
    tty = getFDptr(fdtty);
    if ((tty == NULL) || (tty->refNum == 0)) {
        *ERRNO = EBADF;
...
int KERNpipe(int *ERRNO, int filedes[2])
{
    ...
    pread->refNum = pipen;
    pwrite->refNum = pipen;
    pread->refType = rtPIPE;
    pwrite->refType = rtPIPE;
    pwrite->refLevel = ft->fdLevel | ft->fdLevelMode;
    pread->refLevel = pwrite->refLevel;
    pwrite->NLenableMask = 0;
    pread->NLenableMask = 0;
#endif
