...
int SOCKioctl(void *dataptr, longword tioc, int sock) {
    if (!pr_usrreq)
        return EPFNOSUPPORT;
    return (*pr_usrreq)(sock, PRU_CONTROL, (void *)&tioc, NULL,
                        (struct sockaddr *)dataptr, NULL, NULL);
}
...
int SOCKselect(int pid, int fl, int sock) {
    ...
    (*pr_usrreq)(sock, PRU_SELECT, NULL, (size_t *)&pid, (struct sockaddr *)&fl,
                 NULL, NULL);
    return (fl != 0) ? 1 : 0;
}
...
/* Called from inside GS/OS, has to return special error codes */
{
    ...
    if (!pr_usrreq)
        return EPFNOSUPPORT | 0x4300;
    (*pr_usrreq)(sock, PRU_DISCONNECT, NULL, NULL, NULL, NULL, NULL);
    err = (*pr_usrreq)(sock, PRU_DETACH, NULL, NULL, NULL, NULL, NULL);
...
/* Called from inside GS/OS, has to return special error codes */
{
    ...
    if (!pr_usrreq)
        return 0x4300 | EPFNOSUPPORT;
    len = pb->reqCount;
    err = (*pr_usrreq)(sock, (((cmd & 0xFF) == 0x12) ? PRU_RCVD : PRU_SEND),
                       pb->buffer, &len, NULL, NULL, NULL);
    pb->xferCount = len;
...
pascal int KERNsocket(int domain, int type, int protocol, int *ERRNO) {
    ...
    if (*ERRNO = (*pr_usrreq)(type, PRU_ATTACH, &sock, (size_t *)selwakeup,
                              (struct sockaddr *)&protocol, NULL, NULL))
        return -1;
    disableps();
    fdt = PROC->openFiles;
    fdt->fdCount++;
...
pascal int KERNbind(int fd, struct sockaddr *my_addr, int addrlen, int *ERRNO) {
    ...
    if (*ERRNO =
            (*pr_usrreq)(sock, PRU_BIND, NULL, NULL, my_addr, &addrlen, NULL))
        return -1;
    return 0;
}

pascal int KERNconnect(int fd, struct sockaddr *serv_addr, int addrlen,
                       int *ERRNO) {
    int sock;
    ...
    if (*ERRNO = (*pr_usrreq)(sock, PRU_CONNECT, NULL, NULL, serv_addr,
                              &addrlen, NULL))
        return -1;
    return 0;
}
...
pascal int KERNlisten(int fd, int backlog, int *ERRNO) {
    ...
    if (*ERRNO =
            (*pr_usrreq)(sock, PRU_LISTEN, &backlog, NULL, NULL, NULL, NULL))
        return -1;
    return 0;
}

pascal int KERNaccept(int fd, struct sockaddr *rem_addr, int *addrlen,
                      int *ERRNO) {
    int newfd, sock;
    fdentryPtr fdp;
    fdtablePtr fdt;
    ...
    if (*ERRNO = (*pr_usrreq)(sock, PRU_ACCEPT, &newfd, (size_t *)selwakeup,
                              rem_addr, addrlen, NULL))
        return -1;
    disableps();
    fdt = PROC->openFiles;
    fdt->fdCount++;
...
pascal int KERNrecvfrom(int fd, void *buf, size_t len, unsigned int flags,
                        struct sockaddr *rem_addr, int *addrlen, int *ERRNO) {
    int sock;
    if ((sock = getsocknum(fd)) < 0) {
        *ERRNO = -sock;
        return -1;
    }
    if (*ERRNO = (*pr_usrreq)(sock, (flags & MSG_OOB) ? PRU_RCVOOB : PRU_RCVD,
                              buf, &len, rem_addr, addrlen, NULL))
        return -1;
    return len;
}

pascal int KERNsendto(int fd, void *buf, size_t len, unsigned int flags,
                      struct sockaddr *rem_addr, int addrlen, int *ERRNO) {
    int sock;
    if ((sock = getsocknum(fd)) < 0) {
        *ERRNO = -sock;
    ...
    if (*ERRNO = (*pr_usrreq)(sock, (flags & MSG_OOB) ? PRU_SENDOOB : PRU_SEND,
                              buf, &len, rem_addr, &addrlen, NULL))
        return -1;
    return len;
}

pascal int KERNrecv(int fd, void *buf, size_t len, unsigned int flags,
                    int *ERRNO) {
    int sock;
    if ((sock = getsocknum(fd)) < 0) {
        *ERRNO = -sock;
...
pascal int KERNsend(int fd, void *buf, size_t len, unsigned int flags,
                    int *ERRNO) {
    int sock;
    if ((sock = getsocknum(fd)) < 0) {
        *ERRNO = -sock;
...
pascal int KERNgetpeername(int s, struct sockaddr *peer_addr, int *addrlen,
                           int *ERRNO) {
    int sock;
    ...
    if (*ERRNO = (*pr_usrreq)(sock, PRU_PEERADDR, NULL, NULL, peer_addr,
                              addrlen, NULL))
        return -1;
    return 0;
}

pascal int KERNgetsockname(int s, struct sockaddr *sock_addr, int *addrlen,
                           int *ERRNO) {
    int sock;
    if ((sock = getsocknum(s)) < 0) {
        *ERRNO = -sock;
    ...
    if (*ERRNO = (*pr_usrreq)(sock, PRU_SOCKADDR, NULL, NULL, sock_addr,
                              addrlen, NULL))
        return -1;
    return 0;
}

pascal int KERNgetsockopt(int s, int level, int optname, void *optval,
                          int *optlen, int *ERRNO) {
    int sock;
    if ((sock = getsocknum(s)) < 0) {
        *ERRNO = -sock;
        return -1;
    }
    if (*ERRNO = (*pr_usrreq)(sock, PRU_CO_GETOPT, &level, (size_t *)&optname,
                              (struct sockaddr *)optval, optlen, NULL))
        return -1;
    return 0;
}

pascal int KERNsetsockopt(int s, int level, int optname, void *optval,
                          int optlen, int *ERRNO) {
    int sock;
    if ((sock = getsocknum(s)) < 0) {
        *ERRNO = -sock;
    ...
    if (*ERRNO = (*pr_usrreq)(sock, PRU_CO_SETOPT, &level, (size_t *)&optname,
                              (struct sockaddr *)optval, &optlen, NULL))
        return -1;
    return 0;
}
...
pascal int KERNshutdown(int s, int how, int *ERRNO) {
    ...
    if (*ERRNO = (*pr_usrreq)(sock, PRU_SHUTDOWN, &how, NULL, NULL, NULL, NULL))
        return -1;
    return 0;
}
#pragma toolparms 0
