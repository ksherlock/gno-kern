...
/* remove an item from a list and return it */
{
    ...
    while (test != mptr) {
        if (!test)
            return SYSERR; /* item not found */
        test = test->p_link;
    }

    if (!mptr->p_link)
        nq[qnum].tail = mptr->p_rlink;
    else
        mptr->p_link->p_rlink = mptr->p_rlink;
    if (!mptr->p_rlink)
        nq[qnum].head = mptr->p_link;
    else
        mptr->p_rlink->p_link = mptr->p_link;
...
/* int key;	key to use for this process */
{
    ...
    prev = NULL;
    for (next = nq[qnum].head; next && (next->p_prio < key);
         next = next->p_link)
        prev = next;
    if (mptr->p_rlink = prev)
        prev->p_link = mptr;
    else
        nq[qnum].head = mptr;
    if (mptr->p_link = next)
